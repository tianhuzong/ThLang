...
public:
    ...
    static std::shared_ptr<thlang::Type> create(const std::string& name, bool implicit_copy, size_t size)  ;
...
public:
    std::shared_ptr<thlang::Type> return_type;
    std::vector<std::shared_ptr<thlang::Type>> arg_types;
    FunctionType(std::shared_ptr<thlang::Type> return_type,const  std::vector<std::shared_ptr<thlang::Type>>& arg_types) : Type("函数", false, 0), return_type(return_type), arg_types(std::move(arg_types)){} ; // 函数类型不参与隐式复制
...
private:
    ...
    std::vector<std::pair<std::shared_ptr<thlang::Type>, std::string>> members; 
public:
    ...
    void add_member(std::shared_ptr<thlang::Type> type, const std::string& member_name) ;
    virtual llvm::Type* get_llvm_type(llvm::LLVMContext& context) override;
};
...
private:
    ...
    std::unordered_map<std::string, std::shared_ptr<thlang::Type>> type_map; // 改为指针存储
    llvm::LLVMContext llvmContext;
    static std::shared_ptr<thlang::Type> empty_type_ptr;
public:
    TypeSystem() ;
    
    void add_type(const std::string& type_name, std::shared_ptr<thlang::Type> type) ;
    llvm::LLVMContext& getContext();
    std::shared_ptr<thlang::Type> get_type(const std::string& type_name) ;
