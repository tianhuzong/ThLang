...
void StructType::add_member(Type* type, const std::string& member_name) {
    members.emplace_back(type, member_name);
    size += type->size; // 更新结构体大小
}
...
TypeSystem::TypeSystem(llvm::LLVMContext& llvmContext) : llvmContext(llvmContext)  {
    // 注册基本类型（隐式复制）
